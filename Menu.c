...
menu_t *oled_menu_init(void) {
	...
	menu_t* calibrate_joystick = new_menu("Joystick Calib", main_menu);
	menu_t* set_difficulty = new_menu("Set Difficulty", main_menu);
	
	
	
	menu_t* easy = new_menu("Easy", set_difficulty);
	menu_t* medium = new_menu("Medium", set_difficulty);
	menu_t* hard = new_menu("Hard", set_difficulty);
...
menu_option_t oled_menu_selection() {
	...
	if (direction == UP) {
		...
		if (joystick_button(RBUTTON)) {
			...
			else if (current_menu->title == "Joystick Calib"){
